// ==================== career.js (v5 - FULL OVERHAUL GAME ENGINE) ====================
(function() {
    'use strict';
    ...
    let matchEvents = [];
    let matchMomentum = { home: 0.5, away: 0.5 }; // 0-1
    let currentTick = 0;
    ...
    function setupLineupPitch(homeTeam) {
        ...
        formation.forEach(row => {
            ...
            row.forEach(p => {
                ...
                const startRating = parseFloat((6.0 + Math.random() * 0.5).toFixed(1));
    ...
    function registerGoal(scoringTeam, minute, scoreEl, scorerOverride = null) {
        ...
        } else if (isUserTeam && Math.random() < userScoringShare()) {
            scorer = player.name;
        } else {
    ...
    function simulateOpenPlayAction(minute) {
        ...
        // --- USER ACTION (player involvement) ---
        let userActionChance = 0.08;
        if (['ST', 'CF', 'LW', 'RW', 'CAM'].includes(pos)) userActionChance = 0.14;
        else if (['CM', 'LM', 'RM'].includes(pos)) userActionChance = 0.12;
        else if (['CDM', 'LB', 'RB'].includes(pos)) userActionChance = 0.10;
        else if (['CB', 'GK'].includes(pos)) userActionChance = 0.07;

        if (roll < userActionChance) {
            // User involved in open play
            ...
            if (['ST', 'CF', 'LW', 'RW', 'CAM'].includes(pos)) {
                if (actionType < 0.35) {
                    ...
                    if (onTarget) {
                        ...
                        updateTeammateVisuals(player.name, 0, 0, 0.15);
                        if (Math.random() < 0.3) {
                            // Goal from open play!
                    ...
                    updateTeammateVisuals(player.name, 0, 0, 0.08);
                ...
                if (actionType < 0.3) {
                    ...
                    if (Math.random() < 0.35) {
                        matchStatsTracker.shotsOnTarget++;
                        updateTeammateVisuals(player.name, 0, 0, 0.12);
                        if (Math.random() < 0.2) {
                            registerGoal(player.club, minute, document.getElementById('simScoreDisplay'), player.name);
                        } else {
                            addCommentary(commentaryBank.shot[Math.floor(Math.random() * commentaryBank.shot.length)](player.name), 'neutral');
            ...
            // NPC action / neutral event
            if (Math.random() < 0.02) {
                // NPC goal chance
                const attacking = Math.random() < 0.5 ? matchContext.homeTeam : matchContext.awayTeam;
                const defending = attacking === matchContext.homeTeam ? matchContext.awayTeam : matchContext.homeTeam;
                // Only if not user team or random
        ...
        // Momentum shift based on actions
        if (Math.random() < 0.02) {
            if (globalHomeScore > globalAwayScore) matchMomentum.home = Math.min(1, matchMomentum.home + 0.05);
            else if (globalAwayScore > globalHomeScore) matchMomentum.away = Math.min(1, matchMomentum.away + 0.05);
        }
    }

    // --- DECISION SYSTEM (overhauled with position-specific options) ---
    function triggerDecision(minute, homeTeam, awayTeam, scoreEl) {
        ...
        // Build position-specific scenarios
        if (['ST', 'CF'].includes(position)) {
            scenarios = [
                {
                    title: `🔥 ${minute}' One-on-One!`,
                    desc: `You're through on goal. What do you do?`,
                    opts: [
                        { text: "⚡ Power Shot", risk: "High risk, high reward", type: "SHOT" },
                        { text: "🎯 Place it", risk: "Precision over power", type: "SHOT" },
                        { text: "👟 Pass to teammate", risk: "Unselfish option", type: "PASS" }
                    ]
                },
                {
                    title: `📐 ${minute}' Hold-up Play`,
                    desc: `You receive the ball with back to goal.`,
                    opts: [
                        { text: "🔄 Turn and shoot", risk: "Try to create space", type: "SHOT" },
                        { text: "👟 Lay off", risk: "Combine with a midfielder", type: "PASS" },
                        { text: "💨 Run at defender", risk: "Take him on", type: "DRIBBLE" }
                    ]
                }
            ];
            ...
            scenarios = [
                {
                    ...
                    desc: `You have the ball on the flank.`,
                    opts: [
                        { text: "⚡ Cut Inside & Shoot", risk: "Try to score", type: "SHOT" },
                        { text: "👟 Cross into Box", risk: "Find the striker", type: "PASS" },
                        { text: "💨 Dribble past defender", risk: "Take him on", type: "DRIBBLE" }
                    ]
                },
                {
                    title: `🔀 ${minute}' Overlap Opportunity`,
                    desc: `Your full-back is overlapping.`,
                    opts: [
                        { text: "👟 Pass to overlap", risk: "Create width", type: "PASS" },
                        { text: "⚡ Cut Inside", risk: "Come inside and shoot", type: "SHOT" },
                        { text: "🔄 Keep possession", risk: "Safe option", type: "HOLD" }
                    ]
                }
            ];
            ...
            scenarios = [
                {
                    title: `🎯 ${minute}' Creative Chance`,
                    desc: `You have time on the ball in midfield.`,
                    opts: [
                        { text: "👟 Through Ball", risk: "Try to unlock defence", type: "PASS" },
                        { text: "⚡ Long Shot", risk: "Try from distance", type: "SHOT" },
                        { text: "💨 Dribble Forward", risk: "Carry the ball", type: "DRIBBLE" }
                    ]
                },
                {
                    title: `🔄 ${minute}' Tempo Control`,
                    desc: `The game is frantic. What do you do?`,
                    opts: [
                        { text: "🔄 Slow it down", risk: "Keep possession", type: "HOLD" },
                        { text: "👟 Quick Pass", risk: "Speed up play", type: "PASS" },
                        { text: "⚡ Shoot on Sight", risk: "Catch keeper off guard", type: "SHOT" }
                    ]
                }
            ];
            ...
            scenarios = [
                {
                    ...
                    desc: `You face an attacker.`,
                    opts: [
                        { text: "⚡ Aggressive Tackle", risk: "Win ball, risk foul", type: "TACKLE" },
                        { text: "🔄 Intercept & Pass", risk: "Read and start attack", type: "PASS" },
                        { text: "🛡️ Contain", risk: "Hold position", type: "HOLD" }
                    ]
                },
                {
                    title: `📤 ${minute}' Build from Back`,
                    desc: `You have the ball at the back.`,
                    opts: [
                        { text: "👟 Short Pass", risk: "Safe option", type: "PASS" },
                        { text: "⚡ Long Ball", risk: "Switch play", type: "SHOT" },
                        { text: "💨 Carry Forward", risk: "Join attack", type: "DRIBBLE" }
                    ]
                }
            ];
        } else if (position === 'GK') {
            scenarios = [
                {
                    title: `🧤 ${minute}' One-on-One`,
                    desc: `Striker through on goal.`,
                    opts: [
                        { text: "🧤 Rush Out", risk: "Close angle", type: "SAVE" },
                        { text: "🛡️ Stay Big", risk: "Make yourself big", type: "HOLD" },
                        { text: "👟 Sweep & Clear", risk: "Clear danger", type: "PASS" }
                    ]
                },
                {
                    title: `⚡ ${minute}' Set Piece Defence`,
                    desc: `Organise the wall.`,
                    opts: [
                        { text: "🧤 Come for Cross", risk: "Claim it", type: "SAVE" },
                        { text: "📢 Direct Wall", risk: "Organise defenders", type: "HOLD" },
                        { text: "🔄 Stay on Line", risk: "Wait for shot", type: "HOLD" }
                    ]
                }
            ];
        } else {
            ...
            scenarios = [
                {
                    title: `⚡ ${minute}' Quick Decision`,
                    desc: `What's your move?`,
                    opts: [
                        { text: "⚡ Shoot", risk: "Go for goal", type: "SHOT" },
                        { text: "👟 Pass", risk: "Keep it moving", type: "PASS" },
                        { text: "💨 Dribble", risk: "Take on defender", type: "DRIBBLE" }
                    ]
                }
            ];
        }
    ...
    function decisionSuccessChance(type) {
        ...
        return Math.max(0.25, Math.min(0.75, base + ovrMod + moraleMod));
    }
    ...
    function startLiveMatchSimulation(homeTeam, awayTeam) {
        ...
        matchInterval = setInterval(() => {
            ...
            // Natural goal chance based on expected goals
            if (Math.random() < expectedHome / TOTAL_TICKS / 2) {
                registerGoal(homeTeam, matchMinute, scoreEl);
            }
            if (Math.random() < expectedAway / TOTAL_TICKS / 2) {
                registerGoal(awayTeam, matchMinute, scoreEl);
            }

            // Random neutral event
            if (Math.random() < 0.03) {
