// ==================== career.js (FINAL VERSION - 30s MATCH, HIGH IMPACT TRAINING) ====================
(function() {
    'use strict';
    ...
    // --- 11. MATCH ENGINE (FIXED: SCOREBOARD, VISUALS, TOP SCORERS, 30s MATCH) ---
    ...
    function startLiveMatchSimulation(homeTeam, awayTeam) {
        ...
        let second = 0;
        let decision30Triggered = false, decision50Triggered = false, decision70Triggered = false, decision85Triggered = false;
        let scorers = [], assisters = [];
        ...
        matchInterval = setInterval(() => {
            ...
            second++;
            const matchMinute = Math.min(90, Math.floor((second / 30) * 90)); // 30s match
            clockEl.textContent = `${matchMinute < 10 ? '0' : ''}${matchMinute}:00`;

            // Dynamic Decision Triggers (Min 30, 50, 70, 85)
            if (matchMinute >= 30 && !decision30Triggered) {
                decision30Triggered = true;
                triggerDecision(matchMinute, homeTeam, awayTeam, scoreEl);
            }
            if (matchMinute >= 50 && !decision50Triggered) {
                decision50Triggered = true;
                triggerDecision(matchMinute, homeTeam, awayTeam, scoreEl);
            }
            ...
            if (matchMinute >= 85 && !decision85Triggered) {
                decision85Triggered = true;
                triggerDecision(matchMinute, homeTeam, awayTeam, scoreEl);
            }
            ...
            if (second >= 30) { // 30s match
                isMatchEnded = true;
                clearInterval(matchInterval);
                clockEl.textContent = "90:00";
                addCommentary(`🔔 <strong>TAMAT PERLAWANAN!</strong> ${homeTeam} ${globalHomeScore} - ${globalAwayScore} ${awayTeam}.`, 'card');
    ...
    // --- FIXED: POSITION-SPECIFIC DECISION LOGIC ---
    function triggerDecision(minute, homeTeam, awayTeam, scoreEl) {
        ...
        if (position === 'ST' || position === 'CF') {
            scenarios = [
                {
                    title: `🔥 ${minute}' Peluang Emas Striker!`,
                    desc: `Anda berhadapan dengan penjaga gol! Apa keputusan anda?`,
                    opts: [
                        { text: "⚡ Rembatan Kencang", risk: "Berisiko tinggi, ganjaran besar", type: "SHOT" },
                        { text: "🎯 Hantaran Sisi", risk: "Beri peluang kepada rakan", type: "PASS" },
                        { text: "🛡️ Menjaga Bola", risk: "Kawal bola, tunggu sokongan", type: "HOLD" }
                    ]
                },
                {
                    title: `🥅 ${minute}' Tendangan Bebas!`,
                    desc: `Anda mendapat peluang tendangan bebas di kawasan berbahaya.`,
                    opts: [
                        { text: "⚡ Rembatan Terus", risk: "Cuba skor terus", type: "SHOT" },
                        { text: "🎯 Hantaran ke Kotak", risk: "Cari rakan untuk tandukan", type: "PASS" },
                        { text: "🔄 Hantaran Pendek", risk: "Buka ruang untuk tembakan lain", type: "HOLD" }
                    ]
                }
            ];
        }
        // --- SCENARIOS FOR MIDFIELDERS (CM, CAM, CDM, LM, RM) ---
            ...
            scenarios = [
                {
                    title: `🎯 ${minute}' Peluang Assist!`,
                    desc: `Anda berada di tengah padang dengan bola. Apa anda lakukan?`,
                    opts: [
                        { text: "👟 Hantaran Lorong", risk: "Cuba cari penyerang", type: "PASS" },
                        { text: "⚡ Rembatan Jarak Jauh", risk: "Cuba skor dari jauh", type: "SHOT" },
                        { text: "🛡️ Menjaga Bola", risk: "Kawal bola, kurangkan risiko", type: "HOLD" }
                    ]
                },
                {
                    title: `📐 ${minute}' Sepakan Sudut!`,
                    desc: `Anda akan mengambil sepakan sudut. Siapa sasaran anda?`,
                    opts: [
                        { text: "✈️ Hantaran Tinggi", risk: "Cari pemain tinggi di kotak", type: "PASS" },
                        { text: "⚡ Hantaran Rendah", risk: "Hantaran pantas ke tepi kotak", type: "SHOT" },
                        { text: "🔄 Hantaran Pendek", risk: "Mainkan hantaran pendek", type: "HOLD" }
                    ]
                }
            ];
        }
        // --- SCENARIOS FOR DEFENDERS (CB, LB, RB) ---
            ...
            scenarios = [
                {
                    title: `🛡️ ${minute}' Situasi Pertahanan!`,
                    desc: `Anda berdepan dengan penyerang lawan. Apa tindakan anda?`,
                    opts: [
                        { text: "⚡ Tebing Bola", risk: "Cuba rampas bola", type: "SHOT" },
                        { text: "👟 Hantaran Keluar", risk: "Beri bola kepada rakan", type: "PASS" },
                        { text: "🛡️ Mengawal", risk: "Kawal posisi, jangan biar mereka lepas", type: "HOLD" }
                    ]
                },
                {
                    title: `⚔️ ${minute}' Serangan Balas!`,
                    desc: `Anda mendapat peluang untuk menyerang dari sisi.`,
                    opts: [
                        { text: "💨 Laju ke Hadapan", risk: "Terus mara ke hadapan", type: "SHOT" },
                        { text: "👟 Hantaran Sisi", risk: "Beri bola kepada rakan", type: "PASS" },
                        { text: "⏳ Hentikan Bola", risk: "Perlahankan permainan", type: "HOLD" }
                    ]
                }
            ];
        }
        // --- SCENARIOS FOR GOALKEEPER (GK) ---
            ...
            scenarios = [
                {
                    title: `🧤 ${minute}' Situasi Penjaga Gol!`,
                    desc: `Lawan melakukan serangan. Apa anda lakukan?`,
                    opts: [
                        { text: "🧤 Menyelamat", risk: "Cuba selamatkan gawang", type: "SHOT" },
                        { text: "👟 Hantaran Pantas", risk: "Lepaskan bola cepat", type: "PASS" },
                        { text: "🛡️ Kawal Kotak", risk: "Kawal posisi dengan baik", type: "HOLD" }
                    ]
                },
                {
                    title: `⚡ ${minute}' Situasi Bola Mati!`,
                    desc: `Anda perlu mengatur pertahanan untuk tendangan bebas.`,
                    opts: [
                        { text: "🧤 Posisi", risk: "Atur posisi dengan betul", type: "SHOT" },
                        { text: "📢 Arahan", risk: "Arahkan pemain lain", type: "PASS" },
                        { text: "🛡️ Bersedia", risk: "Bersedia untuk sebarang kemungkinan", type: "HOLD" }
                    ]
                }
            ];
        }
        // --- DEFAULT SCENARIOS FOR WINGERS (LW, RW) ---
            ...
            scenarios = [
                {
                    title: `⚡ ${minute}' Peluang Sayap!`,
                    desc: `Anda berada di sisi padang. Apa anda lakukan?`,
                    opts: [
                        { text: "⚡ Rembatan", risk: "Cuba skor dari sayap", type: "SHOT" },
                        { text: "👟 Hantaran Silang", risk: "Hantar ke kotak penalti", type: "PASS" },
                        { text: "🛡️ Menjaga Bola", risk: "Kawal bola, kurangkan risiko", type: "HOLD" }
                    ]
                }
            ];
        ...
        selectedScenario.opts.forEach(opt => {
            ...
            btn.onclick = () => {
                ...
                // Base success rate
                let success = false;
                if (opt.type === "SHOT" || opt.type === "PASS") {
                    success = Math.random() < 0.45; // 45% chance for shot/pass
                } else {
                    success = Math.random() < 0.8; // 80% chance to hold ball
                }

                if (success) {
                    let goalsDelta = 0, assistsDelta = 0;
                    if (opt.type === "SHOT") {
                        goalsDelta = 1;
                        scorers.push(player.name);
                        playGoalSound();
                        // FIXED: Update global scores directly
                        ...
                        scoreEl.textContent = `${globalHomeScore} - ${globalAwayScore}`;
                        addCommentary(`⚽ ${player.name} menjaringkan gol menerusi keputusan bijak!`, 'goal');
                    } else if (opt.type === "PASS") {
                        assistsDelta = 1;
                        assisters.push(player.name);
                        addCommentary(`👟 ${player.name} membuat hantaran assist yang sempurna!`, 'goal');
                    } else {
                        addCommentary(`🛡️ ${player.name} mengawal situasi dengan bijak.`, 'decision-log');
                    }
                    updateTeammateVisuals(player.name, goalsDelta, assistsDelta, 1.2);
                    addCommentary(`🔥 <strong>BERJAYA!</strong> Tindakan anda mengubah momentum perlawanan!`, 'decision-log');
                } else {
                    addCommentary(`❌ Percubaan anda gagal. Pertahanan lawan berjaya membaca gerakan.`, 'card');
                }
            };
