...
(function() {
    ...
    // --- 3b. CAREER FIELD BOOTSTRAP (fixes NaN weeks / missing abilities on old saves) ---
    ...
    function computeMarketValue() {
        ...
        // Exponential OVR curve, then an age multiplier (peak 21-27)
    ...
    // Abilities grow where the player actually performs
        ...
        if (europeResults) {
            for (const comp in europeResults) {
                ...
                if (compData.winner === player.club) {
                    ...
                    if (!player.trophies.some(t => t.name === trophy.name && t.season === trophy.season)) {
                        player.trophies.push(trophy);
                        addCommentary(`🏆 <strong>${player.club} WIN THE ${comp}!</strong> Season ${season}!`, 'goal');
                    }
                }
            }
        }
    ...
    // --- 11. MATCH ENGINE (v2: 20s MATCH, 3 DECISIONS, TIERED RATINGS) ---
    ...
    // Rating tiers: red 5.0-5.9 | orange 6.0-6.9 | green 7.0-10.0
    ...
    // Club strength derived from its ranking inside the league table of the database
    ...
    function setupLineupPitch(homeTeam) {
        ...
        formation.forEach(row => {
            ...
            row.forEach(p => {
                ...
                // Everybody starts on a neutral 6.0-6.4: a high rating must be earned
    ...
    // Chance that the user is the one who finishes off a goal of his own team
    function userScoringShare() {
        const pos = (player.position || '').toUpperCase();
        if (pos === 'GK') return 0;
        if (pos === 'ST' || pos === 'CF') return 0.34;
        if (['LW', 'RW', 'CAM'].includes(pos)) return 0.24;
        if (['CM', 'LM', 'RM'].includes(pos)) return 0.14;
        return 0.06;
    }
    ...
    function userAssistShare() {
        ...
        if (['CAM', 'CM', 'LW', 'RW', 'LM', 'RM'].includes(pos)) return 0.26;
        if (['LB', 'RB', 'CDM'].includes(pos)) return 0.14;
        if (pos === 'ST' || pos === 'CF') return 0.12;
        return 0.06;
    }
    ...
    function registerGoal(scoringTeam, minute, scoreEl) {
        ...
        if (isUserTeam && Math.random() < userScoringShare()) {
            scorer = player.name;
            addCommentary(`⚽ <strong>GOAL ${minute}'!</strong> ${player.name} finishes it off for ${scoringTeam}!`, 'goal');
        } else {
            scorer = pickScorer(roster);
            addCommentary(`⚽ <strong>GOAL ${minute}'!</strong> ${scoringTeam} score through ${scorer}!`, 'goal');
        }
        matchScorers.push(scorer);
        updateTeammateVisuals(scorer, 1, 0, 0.7);
        ...
        if (Math.random() < 0.68) {
            ...
            if (assister) {
                ...
                updateTeammateVisuals(assister, 0, 1, 0.35);
        ...
        // Conceding hurts the keeper and the back line a little
    ...
    function neutralEvent(minute) {
        ...
        const pool = [
            () => addCommentary(`${minute}' Chance for ${attacking}, the shot flies just wide.`),
            () => addCommentary(`${minute}' Great save by the ${defending} goalkeeper!`),
            () => addCommentary(`${minute}' ${attacking} keep the ball and probe patiently.`),
            () => addCommentary(`${minute}' Corner for ${attacking}, cleared by the ${defending} defence.`),
            () => addCommentary(`🟨 ${minute}' Yellow card for a ${defending} player after a late challenge.`, 'card'),
            () => addCommentary(`${minute}' Counter attack from ${attacking}, but the final pass is poor.`)
        ];
    ...
    function startLiveMatchSimulation(homeTeam, awayTeam) {
        ...
        document.getElementById('simCommentaryBox').innerHTML = `<div class="commentary-item">Kick-off! Matchday ${player.currentMatchWeek} (${activeLeagueName}) is under way.</div>`;
        document.getElementById('btnCloseModal').style.display = 'none';
        modal.classList.add('active');

        setupLineupPitch(homeTeam);

        // Exactly 3 decision moments, spread over the match at random minutes
        ...
        matchInterval = setInterval(() => {
            ...
            if (Math.random() < 0.09) neutralEvent(matchMinute);
    ...
    function finishMatch(homeTeam, awayTeam, clockEl) {
        ...
        const finalRating = clampRating(pStat.rating);
        ...
        if (!player.ratingHistory) player.ratingHistory = [];
        player.ratingHistory.push(finalRating);
        player.lastMatchRating = finalRating;

        // Morale follows performance and result
        ...
        const matchStats = {
            ...
            tackles: isDefensivePlayer ? Math.floor(Math.random() * 4) + 1 : Math.floor(Math.random() * 2),
            interceptions: isDefensivePlayer ? Math.floor(Math.random() * 4) + 1 : Math.floor(Math.random() * 2),
    ...
    // --- POSITION-SPECIFIC DECISIONS (max 3 per match, medium chance, always commentated) ---
    ...
    // Medium success chance (~40-60%), slightly shifted by OVR and morale
    ...
    // Every decision is always commentated so the user knows the outcome
    function resolveDecision(opt, minute, homeTeam, scoreEl) {
        ...
        if (success) {
            ...
            if (opt.type === "SHOT" && position !== 'GK' && !['CB', 'LB', 'RB'].includes(position) && shotGoesIn) {
                ...
                if (scoreEl) scoreEl.textContent = `${globalHomeScore} - ${globalAwayScore}`;
                updateTeammateVisuals(player.name, 1, 0, 0.7);
                addCommentary(`⚽ <strong>GOAL ${minute}'!</strong> ${player.name} takes the chance and scores!`, 'goal');
                ...
                if (scored) {
                    ...
                    if (scoreEl) scoreEl.textContent = `${globalHomeScore} - ${globalAwayScore}`;
                    updateTeammateVisuals(finisher, 1, 0, 0.7);
                    updateTeammateVisuals(player.name, 0, 1, 0.45);
                    playGoalSound();
                    addCommentary(`👟 <strong>ASSIST!</strong> ${player.name} sets up ${finisher} and it is in!`, 'goal');
                } else {
                    updateTeammateVisuals(player.name, 0, 0, 0.25);
                    addCommentary(`✅ ${minute}' Good ball from ${player.name}, but the finish is saved.`, 'decision-log');
                }
            } else if (opt.type === "SHOT") {
                updateTeammateVisuals(player.name, 0, 0, 0.3);
                addCommentary(`✅ ${minute}' ${player.name} forces a fine save from the goalkeeper.`, 'decision-log');
            } else {
                updateTeammateVisuals(player.name, 0, 0, 0.25);
                addCommentary(`✅ ${minute}' ${player.name} reads the situation well and comes out on top.`, 'decision-log');
            }
            addCommentary(`🔥 <strong>SUCCESS!</strong> Your rating is now ${getUserRating().toFixed(1)}.`, 'decision-log');
        } else {
            // A wrong decision costs more than a good one gains: rating is hard to climb
            const penalty = { SHOT: 0.5, PASS: 0.4, HOLD: 0.3 }[opt.type] || 0.4;
            updateTeammateVisuals(player.name, 0, 0, -penalty);
            const failLines = {
                SHOT: `❌ ${minute}' ${player.name} goes for it but the effort is blocked.`,
                PASS: `❌ ${minute}' ${player.name} overhits the pass and possession is lost.`,
                HOLD: `❌ ${minute}' ${player.name} is dispossessed while trying to hold the ball.`
            };
            addCommentary(failLines[opt.type] || `❌ ${minute}' Your attempt failed.`, 'card');
            addCommentary(`📉 <strong>FAILED.</strong> Your rating drops to ${getUserRating().toFixed(1)}.`, 'card');
        }
        saveData();
    }
    ...
    function calculateSeasonOVRChanges() {
        ...
        // Stat milestones are a bonus, not the main driver
        ovrChange *= 0.5;

        // Season average rating drives most of the growth: consistency is what pays
        ...
        // Young players develop faster, veterans decline
        ...
        // Growth gets harder the closer you are to the top
        ...
        // Abilities follow the OVR movement
    ...
    // --- MID-SEASON PROGRESSION (Week 19: smaller version of the end-of-season review) ---
    ...
    function updateUI() {
        const elements = {
            ...
            'careerAvgRating': seasonAverageRating().toFixed(2),
            'careerLastRating': (player.lastMatchRating || 0).toFixed(1)
        };
        ABILITY_KEYS.forEach(k => {
            elements['ability_' + k] = Math.round((player.abilities && player.abilities[k]) || 0);
        });
        ...
        if (avgEl) {
            const tier = ratingTier(seasonAverageRating());
            avgEl.style.color = tier.color;
        }
        const lastEl = document.getElementById('careerLastRating');
        if (lastEl && player.lastMatchRating) {
            lastEl.style.color = ratingTier(player.lastMatchRating).color;
        }
        ...
        // Opponent is locked in for the matchweek so it cannot change mid-match
    ...
    document.getElementById('btnCloseModal').addEventListener('click', () => {
        // Only closable once the match is over, so a match can never be abandoned mid-way
