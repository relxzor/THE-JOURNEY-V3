// ==================== career.js (FIXED: 11 PLAYERS, DYNAMIC FORMATION, POSITION LOCK) ====================
(function() {
    'use strict';
    ...
    // --- 10. TEAM FORMATION GENERATOR (FIXED: 11 PLAYERS, DYNAMIC FORMATION) ---
    function generateTeamFormation(teamName, userPlayer) {
        ...
        // Gantikan pemain user di posisi yang betul
        ...
        // Pilih formasi secara rawak
        const selectedFormation = formations[Math.floor(Math.random() * formations.length)];
        
        // Susun pemain mengikut formasi yang dipilih
        const lineup = selectedFormation.rows.map(row => {
            return row.map(slot => {
                // Cari pemain dengan posisi yang sesuai
                let player = roster.find(p => p.pos === slot.pos);
                if (!player) {
    ...
    function triggerDecision(minute, homeTeam, awayTeam, scoreEl) {
        ...
        // Dynamic Decision Scenarios based on Minute
        const scenarios = [
            {
                title: `🔥 ${minute}' Peluang Emas!`,
                desc: `Anda berada di posisi terbaik untuk menjaringkan gol. Apa yang anda lakukan?`,
                opts: [
                    { text: "🚀 Rembatan Kencang", risk: "Berisiko tinggi, ganjaran besar", type: "SHOT" },
                    { text: "🎯 Hantaran Terukur", risk: "Peluang assist untuk rakan", type: "PASS" },
            ...
            },
            {
                ...
                opts: [
                    ...
                    { text: "🔄 Hantaran Pendek", risk: "Mainkan hantaran pendek untuk bina serangan", type: "HOLD" }
                ]
            },
            {
                title: `⚔️ ${minute}' Serangan Balas!`,
                desc: `Pasukan anda melancarkan serangan balas pantas.`,
                opts: [
                    { text: "💨 Laju ke Hadapan", risk: "Terus lari ke arah gawang", type: "SHOT" },
                    { text: "👟 Hantaran Sisi", risk: "Beri bola kepada rakan yang lebih bebas", type: "PASS" },
                    { text: "⏳ Hentikan Bola", risk: "Perlahankan permainan, kawal rentak", type: "HOLD" }
                ]
            }
        ];

        // Pilih scenario berdasarkan minit
        let selectedScenario;
        if (minute >= 85) {
            selectedScenario = scenarios[3]; // Serangan balas di akhir
        } else if (minute >= 70) {
            selectedScenario = scenarios[1]; // Tendangan bebas di minit lewat
        } else if (minute >= 50) {
            selectedScenario = scenarios[2]; // Sepakan sudut
        } else {
            selectedScenario = scenarios[0]; // Peluang emas awal
        }
