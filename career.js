// ==================== career.js (v6 - FULL ATTRIBUTE INTEGRATION) ====================
(function() {
    'use strict';
    ...
    if (!player.attributes) {
        player.attributes = {
            ...
            agility: 45, injuryResistance: 42
        };
    }

    // Default attribute keys for easy access
            ...
            for (const club in worldLeagues[league]) {
                ...
                if (interest > 25) {
                    ...
                    allClubs.push({
                        club: club,
                        league: league,
                        rating: worldLeagues[league][club]?.rating || 70,
    ...
    // ========================================================================
    // ================ ENJIN PERLAWANAN BARU 100% (INTEGRASI ATTRIBUT) ================
    // ========================================================================
    ...
    let matchEvents = [];
    let matchMomentum = { home: 0.5, away: 0.5 };
    let currentTick = 0;
    let matchMinute = 0;

    // ----- HELPER: GET ATTRIBUTE VALUE -----
    function getAttr(key) {
        return player.attributes[key] || 40;
    }

    // ----- HELPER: COMPUTE USER SKILL MULTIPLIER -----
    function getUserSkillMultiplier(action) {
        const attrs = player.attributes || {};
        let skill = 0;
        switch(action) {
            case 'shooting': skill = (attrs.finishing || 40) * 0.4 + (attrs.composure || 40) * 0.3 + (attrs.ballControl || 40) * 0.2 + (attrs.strength || 40) * 0.1; break;
            case 'passing': skill = (attrs.passing || 40) * 0.4 + (attrs.vision || 40) * 0.3 + (attrs.ballControl || 40) * 0.2 + (attrs.composure || 40) * 0.1; break;
            case 'dribbling': skill = (attrs.dribbling || 40) * 0.3 + (attrs.ballControl || 40) * 0.25 + (attrs.acceleration || 40) * 0.2 + (attrs.agility || 40) * 0.15 + (attrs.doubleTouch || 40) * 0.1; break;
            case 'defending': skill = (attrs.strength || 40) * 0.3 + (attrs.stamina || 40) * 0.2 + (attrs.acceleration || 40) * 0.2 + (attrs.agility || 40) * 0.15 + (attrs.ballControl || 40) * 0.15; break;
            case 'pace': skill = (attrs.sprintSpeed || 40) * 0.5 + (attrs.acceleration || 40) * 0.3 + (attrs.agility || 40) * 0.2; break;
            case 'stamina': skill = attrs.stamina || 40; break;
            default: skill = (attrs.ballControl || 40) * 0.3 + (attrs.composure || 40) * 0.3 + (attrs.vision || 40) * 0.2 + (attrs.agility || 40) * 0.2;
        }
        // Normalize to 0.2 - 1.2 multiplier
        return 0.2 + (skill / 99) * 1.0;
    }

    // ----- HELPER: COMPUTE OVR-BASED PERFORMANCE -----
    function getPerformanceMultiplier() {
        const ovr = player.ovr || 60;
        const morale = (player.morale || 70) / 100;
        const stamina = (player.attributes?.stamina || 45) / 100;
        const fatigue = 1 - ((player.fatigue || 0) / 200);
        const base = ovr / 99;
        return Math.max(0.15, Math.min(1.0, base * morale * stamina * fatigue));
    }

    function ratingTier(rating) {
        if (rating >= 7.0) return { cls: 'rating-good', color: '#21ba45', label: 'Good' };
        if (rating >= 6.0) return { cls: 'rating-average', color: '#f2711c', label: 'Average' };
        return { cls: 'rating-bad', color: '#db2828', label: 'Poor' };
    }
    ...
    function setupLineupPitch(homeTeam) {
        ...
        formation.forEach(row => {
            ...
            row.forEach(p => {
                ...
                const startRating = parseFloat((6.0 + (getPerformanceMultiplier() * 0.6) + Math.random() * 0.3).toFixed(1));
    ...
    // ----- USER SCORING/ASSIST SHARE BASED ON ATTRIBUTES -----
    function userScoringShare() {
        const pos = (player.position || '').toUpperCase();
        const base = {
            'GK': 0.01, 'CB': 0.02, 'LB': 0.04, 'RB': 0.04,
            'CDM': 0.06, 'CM': 0.12, 'CAM': 0.20,
            'LM': 0.14, 'RM': 0.14, 'LW': 0.25, 'RW': 0.25,
            'ST': 0.35, 'CF': 0.30
        }[pos] || 0.08;
        // Boost by finishing attribute
        const finishingBoost = (getAttr('finishing') - 40) / 300;
        return Math.min(0.55, base + finishingBoost);
    }

    function userAssistShare() {
        const pos = (player.position || '').toUpperCase();
        const base = {
            'GK': 0.01, 'CB': 0.02, 'LB': 0.08, 'RB': 0.08,
            'CDM': 0.12, 'CM': 0.20, 'CAM': 0.28,
            'LM': 0.18, 'RM': 0.18, 'LW': 0.16, 'RW': 0.16,
            'ST': 0.12, 'CF': 0.14
        }[pos] || 0.10;
        // Boost by passing and vision
        const passingBoost = ((getAttr('passing') + getAttr('vision')) / 2 - 40) / 300;
        return Math.min(0.50, base + passingBoost);
    }

    // ----- EXPANDED COMMENTARY -----
    const commentaryBank = {
        goal: [
            (scorer, team) => `⚽ <strong>GOOOAL ${getMatchMinute()}'!</strong> ${scorer} rifles it in for ${team}!`,
            (scorer, team) => `⚽ <strong>GOAL ${getMatchMinute()}'!</strong> ${scorer} with a brilliant finish — ${team} take the lead!`,
            (scorer, team) => `⚽ <strong>GOAL ${getMatchMinute()}'!</strong> ${scorer} slots it home! What a moment for ${team}!`,
            (scorer, team) => `⚽ <strong>GOAL ${getMatchMinute()}'!</strong> Unstoppable from ${scorer}! ${team} are celebrating!`,
            (scorer, team) => `⚽ <strong>GOAL ${getMatchMinute()}'!</strong> ${scorer} makes no mistake — ${team} strike!`,
            (scorer, team) => `⚽ <strong>GOAL ${getMatchMinute()}'!</strong> ${scorer} with a spectacular finish! ${team} fans are ecstatic!`
        ],
        assist: (assister) => `👟 Assist by ${assister}.`,
        shot: [
            (player) => `${getMatchMinute()}' ${player} unleashes a shot — saved by the keeper!`,
            (player) => `${getMatchMinute()}' ${player} tries from distance — just over the bar!`,
            (player) => `${getMatchMinute()}' ${player} fires wide — good chance wasted.`,
            (player) => `${getMatchMinute()}' ${player} gets a shot away — the keeper gathers comfortably.`,
            (player) => `${getMatchMinute()}' ${player} with a powerful strike — the keeper tips it over!`
        ],
        tackle: [
            (player) => `${getMatchMinute()}' ${player} with a crunching tackle to win the ball!`,
            (player) => `${getMatchMinute()}' ${player} reads the play perfectly and intercepts.`,
            (player) => `${getMatchMinute()}' Strong challenge from ${player} — clean and effective.`,
            (player) => `${getMatchMinute()}' ${player} slides in and wins the ball brilliantly!`
        ],
        pass: [
            (player) => `${getMatchMinute()}' ${player} sprays a beautiful pass to switch the play.`,
            (player) => `${getMatchMinute()}' ${player} threads a clever through ball — almost breaks the line.`,
            (player) => `${getMatchMinute()}' ${player} keeps possession with a calm pass.`,
            (player) => `${getMatchMinute()}' ${player} with a sublime cross-field ball!`
        ],
        dribble: [
            (player) => `${getMatchMinute()}' ${player} glides past two defenders with ease!`,
            (player) => `${getMatchMinute()}' ${player} uses quick feet to beat his man.`,
            (player) => `${getMatchMinute()}' ${player} drives forward — a confident run!`,
            (player) => `${getMatchMinute()}' ${player} nutmegs the defender! Brilliant skill!`
        ],
        save: [
            (player) => `${getMatchMinute()}' ${player} makes a crucial save to deny the opposition!`,
            (player) => `${getMatchMinute()}' ${player} tips it wide — great reflexes!`,
            (player) => `${getMatchMinute()}' ${player} stays big and makes the stop.`,
            (player) => `${getMatchMinute()}' ${player} with a world-class save!`
        ],
        neutral: [
            (team) => `${getMatchMinute()}' ${team} build from the back patiently.`,
            (team) => `${getMatchMinute()}' The ball is worked wide by ${team}.`,
            (team) => `${getMatchMinute()}' ${team} win a corner — dangerous delivery.`,
            (team) => `${getMatchMinute()}' Quick counter from ${team} — but the final pass is lacking.`,
            (team) => `${getMatchMinute()}' ${team} are dominating possession at the moment.`
        ]
    };

    function getMatchMinute() {
        return Math.min(90, Math.round((currentTick / TOTAL_TICKS) * 90));
    }
    ...
    // ----- REGISTER GOAL WITH ATTRIBUTE INFLUENCE -----
    function registerGoal(scoringTeam, minute, scoreEl, scorerOverride = null) {
        ...
        if (scorerOverride) {
            scorer = scorerOverride;
        } else if (isUserTeam && Math.random() < userScoringShare() * getPerformanceMultiplier()) {
            scorer = player.name;
        } else {
        ...
        matchEvents.push({ type: 'goal', scorer, team: scoringTeam, minute });

        // Rating boost based on attributes
        const boost = scorer === player.name ? 1.2 * (1 + (getAttr('finishing') - 40) / 200) : 1.0;
        updateTeammateVisuals(scorer, 1, 0, boost);
        
        if (scorer === player.name) {
            playGoalSound();
            const goalLines = commentaryBank.goal;
            const line = goalLines[Math.floor(Math.random() * goalLines.length)](scorer, scoringTeam);
            addCommentary(line, 'goal');
            // Boost stamina usage for goals
            player.attributes.stamina = Math.max(10, (player.attributes.stamina || 45) - 2);
        } else {
            const goalLines = commentaryBank.goal;
            const line = goalLines[Math.floor(Math.random() * goalLines.length)](scorer, scoringTeam);
            addCommentary(line, 'goal');
        ...
        // Assist with attribute influence
        let assister = null;
        if (Math.random() < 0.68) {
            if (isUserTeam && scorer !== player.name && Math.random() < userAssistShare() * getPerformanceMultiplier()) {
            ...
            if (assister) {
                ...
                matchEvents.push({ type: 'assist', assister, scorer, team: scoringTeam, minute });
                const assistBoost = assister === player.name ? 0.7 * (1 + (getAttr('passing') - 40) / 300) : 0.7;
                updateTeammateVisuals(assister, 0, 1, assistBoost);
                const assistLine = commentaryBank.assist(assister);
                addCommentary(assistLine, 'goal');
                if (assister === player.name) {
                    player.attributes.stamina = Math.max(10, (player.attributes.stamina || 45) - 1);
                }
            }
        }
        ...
        // Penalty for conceding team's defensive players
        ...
        if (concedingTeam === player.club) {
            Object.keys(matchPlayerStats).forEach(name => {
                ...
                if (pos === 'GK') updateTeammateVisuals(name, 0, 0, -0.4);
                else if (['CB', 'LB', 'RB', 'CDM'].includes(pos)) updateTeammateVisuals(name, 0, 0, -0.15);
            });
        }
    }

    // ----- SIMULATE OPEN PLAY WITH ATTRIBUTES -----
    function simulateOpenPlayAction(minute) {
        const pos = (player.position || '').toUpperCase();
        const roll = Math.random();
        ...
        // Stamina affects action chance
        const staminaFactor = Math.max(0.3, stamina / 99);
        let userActionChance = 0.08 * staminaFactor * perfMult;
        if (['ST', 'CF', 'LW', 'RW', 'CAM'].includes(pos)) userActionChance = 0.16 * staminaFactor * perfMult;
        else if (['CM', 'LM', 'RM'].includes(pos)) userActionChance = 0.14 * staminaFactor * perfMult;
        else if (['CDM', 'LB', 'RB'].includes(pos)) userActionChance = 0.12 * staminaFactor * perfMult;
        else if (['CB', 'GK'].includes(pos)) userActionChance = 0.08 * staminaFactor * perfMult;

        // Reduce stamina over time
        if (Math.random() < 0.005) {
            player.attributes.stamina = Math.max(5, stamina - 0.5);
        }

        if (roll < userActionChance) {
            const actionType = Math.random();
            const paceMult = getUserSkillMultiplier('pace');
            const dribbleMult = getUserSkillMultiplier('dribbling');
            const shootMult = getUserSkillMultiplier('shooting');
            const passMult = getUserSkillMultiplier('passing');
            const defendMult = getUserSkillMultiplier('defending');

            if (['ST', 'CF', 'LW', 'RW', 'CAM'].includes(pos)) {
                if (actionType < 0.35) {
                    // Shot
                    matchStatsTracker.shots++;
                    const onTarget = Math.random() < (0.35 + (getAttr('finishing') / 300));
                    if (onTarget) {
                        matchStatsTracker.shotsOnTarget++;
                        const ratingBoost = 0.12 * shootMult;
                        updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                        const goalChance = 0.2 + (getAttr('finishing') / 300) + (getAttr('composure') / 400);
                        if (Math.random() < goalChance * perfMult) {
                            registerGoal(player.club, minute, document.getElementById('simScoreDisplay'), player.name);
                            return;
                        } else {
                            const shotLines = commentaryBank.shot;
                            addCommentary(shotLines[Math.floor(Math.random() * shotLines.length)](player.name), 'neutral');
                        }
                    } else {
                        addCommentary(commentaryBank.shot[Math.floor(Math.random() * commentaryBank.shot.length)](player.name), 'neutral');
                    }
                } else if (actionType < 0.65) {
                    // Dribble
                    matchStatsTracker.dribbles++;
                    const ratingBoost = 0.08 * dribbleMult;
                    updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    const dribLines = commentaryBank.dribble;
                    addCommentary(dribLines[Math.floor(Math.random() * dribLines.length)](player.name), 'neutral');
                    // Dribble uses stamina
                    player.attributes.stamina = Math.max(5, stamina - 0.5);
                } else {
                    // Pass
                    matchStatsTracker.passes += 2;
                    const key = Math.random() < (0.25 + (getAttr('vision') / 300));
                    if (key) {
                        matchStatsTracker.keyPasses++;
                        const ratingBoost = 0.10 * passMult;
                        updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    }
                    const passLines = commentaryBank.pass;
                    addCommentary(passLines[Math.floor(Math.random() * passLines.length)](player.name), 'neutral');
                }
            } else if (['CM', 'LM', 'RM'].includes(pos)) {
                if (actionType < 0.3) {
                    // Shot
                    matchStatsTracker.shots++;
                    if (Math.random() < (0.3 + (getAttr('finishing') / 350))) {
                        matchStatsTracker.shotsOnTarget++;
                        const ratingBoost = 0.10 * shootMult;
                        updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                        if (Math.random() < (0.12 + (getAttr('finishing') / 400)) * perfMult) {
                            registerGoal(player.club, minute, document.getElementById('simScoreDisplay'), player.name);
                            return;
                        } else {
                            addCommentary(commentaryBank.shot[Math.floor(Math.random() * commentaryBank.shot.length)](player.name), 'neutral');
                    ...
                    const key = Math.random() < (0.3 + (getAttr('vision') / 280));
                    if (key) {
                        matchStatsTracker.keyPasses++;
                        const ratingBoost = 0.10 * passMult;
                        updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    }
                    addCommentary(commentaryBank.pass[Math.floor(Math.random() * commentaryBank.pass.length)](player.name), 'neutral');
                } else {
                    // Tackle/intercept
                    matchStatsTracker.tackles++;
                    matchStatsTracker.interceptions++;
                    const ratingBoost = 0.10 * defendMult;
                    updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    addCommentary(commentaryBank.tackle[Math.floor(Math.random() * commentaryBank.tackle.length)](player.name), 'neutral');
                }
            } else if (['CDM', 'LB', 'RB', 'CB'].includes(pos)) {
                if (actionType < 0.5) {
                    // Defensive action
                    matchStatsTracker.tackles++;
                    matchStatsTracker.interceptions++;
                    const ratingBoost = 0.12 * defendMult;
                    updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    addCommentary(commentaryBank.tackle[Math.floor(Math.random() * commentaryBank.tackle.length)](player.name), 'neutral');
                    player.attributes.stamina = Math.max(5, stamina - 0.8);
                } else if (actionType < 0.75) {
                    // Pass
                    matchStatsTracker.passes += 2;
                    if (Math.random() < (0.15 + (getAttr('vision') / 350))) {
                        matchStatsTracker.keyPasses++;
                        const ratingBoost = 0.08 * passMult;
                        updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    }
                    addCommentary(commentaryBank.pass[Math.floor(Math.random() * commentaryBank.pass.length)](player.name), 'neutral');
                } else {
                    // Clearance
                    matchStatsTracker.dribbles++;
                    const ratingBoost = 0.05;
                    updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    addCommentary(`${minute}' ${player.name} clears the danger with authority.`, 'neutral');
                }
            } else if (pos === 'GK') {
                if (actionType < 0.3) {
                    matchStatsTracker.saves++;
                    const ratingBoost = 0.15 * (1 + (getAttr('ballControl') / 400));
                    updateTeammateVisuals(player.name, 0, 0, ratingBoost);
                    addCommentary(commentaryBank.save[Math.floor(Math.random() * commentaryBank.save.length)](player.name), 'neutral');
                } else {
                    matchStatsTracker.passes++;
                    addCommentary(`${minute}' ${player.name} distributes the ball calmly.`, 'neutral');
                }
            }
        } else {
            // NPC action
            if (Math.random() < 0.015 * (1 + (globalHomeScore + globalAwayScore) / 20)) {
                const attacking = Math.random() < 0.5 ? matchContext.homeTeam : matchContext.awayTeam;
                if (attacking !== player.club || Math.random() < 0.3) {
                    registerGoal(attacking, minute, document.getElementById('simScoreDisplay'));
                }
            } else if (Math.random() < 0.03) {
                const team = Math.random() < 0.5 ? matchContext.homeTeam : matchContext.awayTeam;
                const neutralLines = commentaryBank.neutral;
                addCommentary(neutralLines[Math.floor(Math.random() * neutralLines.length)](team), 'neutral');
            }
        }

        // Momentum based on score
        if (Math.random() < 0.015) {
            if (globalHomeScore > globalAwayScore) matchMomentum.home = Math.min(1, matchMomentum.home + 0.03);
            else if (globalAwayScore > globalHomeScore) matchMomentum.away = Math.min(1, matchMomentum.away + 0.03);
        }
    }

    // ----- DECISION SYSTEM (position-specific with attribute influence) -----
    function triggerDecision(minute, homeTeam, awayTeam, scoreEl) {
        ...
        const perfMult = getPerformanceMultiplier();
        const shootSkill = getUserSkillMultiplier('shooting');
        const passSkill = getUserSkillMultiplier('passing');
        const dribbleSkill = getUserSkillMultiplier('dribbling');
        const defendSkill = getUserSkillMultiplier('defending');

        let scenarios = [];

        if (['ST', 'CF'].includes(position)) {
            scenarios = [
                { title: `🔥 ${minute}' One-on-One!`, desc: `You're through on goal. What do you do?`,
                  opts: [
                      { text: "⚡ Power Shot", risk: `High risk (${Math.round(45 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "🎯 Place it", risk: `Precision (${Math.round(50 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "👟 Pass to teammate", risk: `Unselfish (${Math.round(55 * passSkill + 10)}%)`, type: "PASS" }
                  ] },
                { title: `📐 ${minute}' Hold-up Play`, desc: `You receive the ball with back to goal.`,
                  opts: [
                      { text: "🔄 Turn and shoot", risk: `Try to create space (${Math.round(40 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "👟 Lay off", risk: `Combine with a midfielder (${Math.round(55 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "💨 Run at defender", risk: `Take him on (${Math.round(50 * dribbleSkill + 10)}%)`, type: "DRIBBLE" }
                  ] }
            ];
        } else if (['LW', 'RW'].includes(position)) {
            scenarios = [
                { title: `💨 ${minute}' Wing Duel!`, desc: `You have the ball on the flank.`,
                  opts: [
                      { text: "⚡ Cut Inside & Shoot", risk: `Try to score (${Math.round(40 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "👟 Cross into Box", risk: `Find the striker (${Math.round(50 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "💨 Dribble past defender", risk: `Take him on (${Math.round(50 * dribbleSkill + 10)}%)`, type: "DRIBBLE" }
                  ] },
                { title: `🔀 ${minute}' Overlap Opportunity`, desc: `Your full-back is overlapping.`,
                  opts: [
                      { text: "👟 Pass to overlap", risk: `Create width (${Math.round(55 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "⚡ Cut Inside", risk: `Come inside and shoot (${Math.round(45 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "🔄 Keep possession", risk: `Safe option (${Math.round(65)}%)`, type: "HOLD" }
                  ] }
            ];
        } else if (['CAM', 'CM'].includes(position)) {
            scenarios = [
                { title: `🎯 ${minute}' Creative Chance`, desc: `You have time on the ball in midfield.`,
                  opts: [
                      { text: "👟 Through Ball", risk: `Try to unlock defence (${Math.round(50 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "⚡ Long Shot", risk: `Try from distance (${Math.round(35 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "💨 Dribble Forward", risk: `Carry the ball (${Math.round(50 * dribbleSkill + 10)}%)`, type: "DRIBBLE" }
                  ] },
                { title: `🔄 ${minute}' Tempo Control`, desc: `The game is frantic. What do you do?`,
                  opts: [
                      { text: "🔄 Slow it down", risk: `Keep possession (${Math.round(65)}%)`, type: "HOLD" },
                      { text: "👟 Quick Pass", risk: `Speed up play (${Math.round(55 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "⚡ Shoot on Sight", risk: `Catch keeper off guard (${Math.round(35 * shootSkill + 10)}%)`, type: "SHOT" }
                  ] }
            ];
        } else if (['CDM', 'CB', 'LB', 'RB'].includes(position)) {
            scenarios = [
                { title: `🛡️ ${minute}' Defensive Duel`, desc: `You face an attacker.`,
                  opts: [
                      { text: "⚡ Aggressive Tackle", risk: `Win ball, risk foul (${Math.round(55 * defendSkill + 10)}%)`, type: "TACKLE" },
                      { text: "🔄 Intercept & Pass", risk: `Read and start attack (${Math.round(50 * defendSkill + 10)}%)`, type: "PASS" },
                      { text: "🛡️ Contain", risk: `Hold position (${Math.round(60)}%)`, type: "HOLD" }
                  ] },
                { title: `📤 ${minute}' Build from Back`, desc: `You have the ball at the back.`,
                  opts: [
                      { text: "👟 Short Pass", risk: `Safe option (${Math.round(65)}%)`, type: "PASS" },
                      { text: "⚡ Long Ball", risk: `Switch play (${Math.round(45 * passSkill + 10)}%)`, type: "SHOT" },
                      { text: "💨 Carry Forward", risk: `Join attack (${Math.round(45 * dribbleSkill + 10)}%)`, type: "DRIBBLE" }
                  ] }
            ];
        } else if (position === 'GK') {
            scenarios = [
                { title: `🧤 ${minute}' One-on-One`, desc: `Striker through on goal.`,
                  opts: [
                      { text: "🧤 Rush Out", risk: `Close angle (${Math.round(55 + getAttr('composure') / 200 * 10)}%)`, type: "SAVE" },
                      { text: "🛡️ Stay Big", risk: `Make yourself big (${Math.round(60 + getAttr('composure') / 200 * 10)}%)`, type: "HOLD" },
                      { text: "👟 Sweep & Clear", risk: `Clear danger (${Math.round(50 + getAttr('ballControl') / 200 * 10)}%)`, type: "PASS" }
                  ] },
                { title: `⚡ ${minute}' Set Piece Defence`, desc: `Organise the wall.`,
                  opts: [
                      { text: "🧤 Come for Cross", risk: `Claim it (${Math.round(50 + getAttr('ballControl') / 200 * 10)}%)`, type: "SAVE" },
                      { text: "📢 Direct Wall", risk: `Organise defenders (${Math.round(60)}%)`, type: "HOLD" },
                      { text: "🔄 Stay on Line", risk: `Wait for shot (${Math.round(65)}%)`, type: "HOLD" }
                  ] }
            ];
        } else {
            scenarios = [
                { title: `⚡ ${minute}' Quick Decision`, desc: `What's your move?`,
                  opts: [
                      { text: "⚡ Shoot", risk: `Go for goal (${Math.round(40 * shootSkill + 10)}%)`, type: "SHOT" },
                      { text: "👟 Pass", risk: `Keep it moving (${Math.round(55 * passSkill + 10)}%)`, type: "PASS" },
                      { text: "💨 Dribble", risk: `Take on defender (${Math.round(50 * dribbleSkill + 10)}%)`, type: "DRIBBLE" }
                  ] }
            ];
        }
    ...
    function decisionSuccessChance(type) {
        const base = { SHOT: 0.42, PASS: 0.52, HOLD: 0.60, DRIBBLE: 0.48, TACKLE: 0.55, SAVE: 0.50 }[type] || 0.5;
        const ovrMod = (getPerformanceMultiplier() - 0.5) * 0.6;
        const moraleMod = (((player.morale || 70) - 70) / 100) * 0.1;
        return Math.max(0.15, Math.min(0.85, base + ovrMod + moraleMod));
    }
    ...
    function resolveDecision(opt, minute, homeTeam, scoreEl) {
        ...
        if (success) {
            let ratingBoost = 0.2;
            if (opt.type === 'SHOT' && ['ST','CF','LW','RW','CAM'].includes(position)) {
                ratingBoost = 0.4 + (getAttr('finishing') - 40) / 200;
                if (Math.random() < (0.3 + (getAttr('finishing') / 300)) * getPerformanceMultiplier()) {
                    registerGoal(player.club, minute, scoreEl, player.name);
                    addCommentary(`🔥 <strong>SUCCESS!</strong> Your decision leads to a goal!`, 'goal');
                    updateTeammateVisuals(player.name, 0, 0, 0.8);
                    return;
                }
            } else if (opt.type === 'PASS' && ['CAM','CM','LW','RW'].includes(position)) {
                ratingBoost = 0.35 + (getAttr('passing') - 40) / 300;
                if (Math.random() < (0.2 + (getAttr('vision') / 400)) * getPerformanceMultiplier()) {
                    const leaguePool = worldLeagues[activeLeagueName] || worldLeagues["Premier League"];
                    const roster = leaguePool[player.club] || [];
                    const finisher = pickScorer(roster.filter(p => p.name !== player.name));
                    if (finisher) {
                        matchScorers.push(finisher);
                        matchAssisters.push(player.name);
                        if (player.club === matchContext.homeTeam) globalHomeScore++;
                        else globalAwayScore++;
                        if (scoreEl) scoreEl.textContent = `${globalHomeScore} - ${globalAwayScore}`;
                        updateTeammateVisuals(finisher, 1, 0, 1.2);
                        updateTeammateVisuals(player.name, 0, 1, 0.7);
                        playGoalSound();
                        addCommentary(`👟 <strong>BRILLIANT ASSIST!</strong> ${player.name} picks out ${finisher} for the goal!`, 'goal');
                        return;
                    }
                }
            } else if (opt.type === 'DRIBBLE' && ['LW','RW','CAM','ST'].includes(position)) {
                ratingBoost = 0.3 + (getAttr('dribbling') - 40) / 300;
                if (Math.random() < (0.15 + (getAttr('dribbling') / 400)) * getPerformanceMultiplier()) {
                    addCommentary(`💨 ${minute}' ${player.name} beats two defenders and creates a great chance!`, 'goal');
                }
            } else if (opt.type === 'TACKLE' && ['CDM','CB','LB','RB'].includes(position)) {
                ratingBoost = 0.35 + (getAttr('strength') - 40) / 300;
                matchStatsTracker.tackles++;
                addCommentary(`🛡️ ${minute}' ${player.name} wins the ball cleanly!`, 'neutral');
            } else if (opt.type === 'SAVE' && position === 'GK') {
                ratingBoost = 0.4 + (getAttr('ballControl') - 40) / 300;
                matchStatsTracker.saves++;
                addCommentary(`🧤 ${minute}' ${player.name} makes a great save!`, 'neutral');
            }
            updateTeammateVisuals(player.name, 0, 0, ratingBoost);
            addCommentary(`✅ <strong>SUCCESS!</strong> Your decision pays off — rating +${ratingBoost.toFixed(1)}.`, 'decision-log');
        } else {
            const penalty = { SHOT: 0.4, PASS: 0.3, HOLD: 0.2, DRIBBLE: 0.35, TACKLE: 0.3, SAVE: 0.3 }[opt.type] || 0.3;
            updateTeammateVisuals(player.name, 0, 0, -penalty);
            const failLines = {
                SHOT: `❌ ${minute}' ${player.name} misses the target.`,
                PASS: `❌ ${minute}' ${player.name} gives the ball away.`,
                HOLD: `❌ ${minute}' ${player.name} is dispossessed.`,
                DRIBBLE: `❌ ${minute}' ${player.name} loses the ball.`,
                TACKLE: `❌ ${minute}' ${player.name} commits a foul.`,
                SAVE: `❌ ${minute}' ${player.name} fails to stop the shot.`
            };
            addCommentary(failLines[opt.type] || `❌ ${minute}' Your decision fails.`, 'card');
            addCommentary(`📉 <strong>UNSUCCESSFUL.</strong> Rating -${penalty.toFixed(1)}.`, 'card');
        }
        saveData();
    }
    ...
    function startLiveMatchSimulation(homeTeam, awayTeam) {
        ...
        decisionsUsed = 0;
        matchEvents = [];
        matchMomentum = { home: 0.5, away: 0.5 };
        currentTick = 0;
        matchStatsTracker = {};

        // Update OVR from attributes before match
        player.ovr = computeOVRFromAttributes();
        saveData();
        ...
        for (let i = 0; i < MAX_DECISIONS_PER_MATCH; i++) {
            decisionMinutes.push(15 + Math.floor(Math.random() * 60));
        }
        decisionMinutes.sort((a,b) => a-b);
        let decisionIndex = 0;
        ...
        matchInterval = setInterval(() => {
            ...
            currentTick++;
            matchMinute = Math.min(90, Math.round((currentTick / TOTAL_TICKS) * 90));
            ...
            simulateOpenPlayAction(matchMinute);

            // Natural goals based on expected goals and performance
            const homeGoalChance = expectedHome / TOTAL_TICKS * (1 + (matchMomentum.home - 0.5) * 0.3);
            const awayGoalChance = expectedAway / TOTAL_TICKS * (1 + (matchMomentum.away - 0.5) * 0.3);
            
            if (Math.random() < homeGoalChance) {
                registerGoal(homeTeam, matchMinute, scoreEl);
            }
            if (Math.random() < awayGoalChance) {
                registerGoal(awayTeam, matchMinute, scoreEl);
            }
    ...
    function finishMatch(homeTeam, awayTeam, clockEl) {
        ...
        addCommentary(`⭐ <strong>Your match rating: ${finalRating.toFixed(1)}</strong> (${tier.label}).`, 'decision-log');
        ...
        // Stamina recovery
        player.attributes.stamina = Math.min(99, (player.attributes.stamina || 45) + 5);
        player.fatigue = Math.max(0, (player.fatigue || 0) - 2);

        let moraleDelta = goalsFor > goalsAgainst ? 5 : (goalsFor === goalsAgainst ? 2 : -4);
        ...
        // Update OVR from attributes after match
        player.ovr = computeOVRFromAttributes();
    ...
    // --- 12. OVR CALCULATION & AGING (with attribute sync) ---
    function calculateSeasonOVRChanges() {
        ...
        // Position-specific attribute growth
        if (['ST', 'CF'].includes(position)) {
            ...
            // Grow attributes
            if (goals > 0) {
                attrs.finishing = Math.min(99, (attrs.finishing || 40) + 0.5 * Math.min(5, goals));
                attrs.composure = Math.min(99, (attrs.composure || 40) + 0.3 * Math.min(5, goals));
            }
            ...
            if (goals > 0) attrs.finishing = Math.min(99, (attrs.finishing || 40) + 0.3 * Math.min(5, goals));
            if (assists > 0) {
                attrs.passing = Math.min(99, (attrs.passing || 40) + 0.3 * Math.min(5, assists));
                attrs.vision = Math.min(99, (attrs.vision || 40) + 0.3 * Math.min(5, assists));
            }
        } else if (['CAM', 'CM'].includes(position)) {
            if (assists >= 3) ovrChange += 0.5;
            if (assists >= 5) ovrChange += 0.5;
            ...
            if (goals >= 3) ovrChange += 0.5;
            if (assists > 0) {
                attrs.passing = Math.min(99, (attrs.passing || 40) + 0.4 * Math.min(5, assists));
                attrs.vision = Math.min(99, (attrs.vision || 40) + 0.4 * Math.min(5, assists));
            }
            ...
            if (tackles > 0) {
                attrs.strength = Math.min(99, (attrs.strength || 40) + 0.3 * Math.min(5, tackles / 3));
                attrs.stamina = Math.min(99, (attrs.stamina || 40) + 0.2 * Math.min(5, tackles / 3));
            }
            ...
            if (tackles > 0) attrs.strength = Math.min(99, (attrs.strength || 40) + 0.3 * Math.min(5, tackles / 3));
            if (cleanSheets > 0) attrs.composure = Math.min(99, (attrs.composure || 40) + 0.2 * Math.min(5, cleanSheets));
        } else if (position === 'GK') {
            if (cleanSheets >= 3) ovrChange += 0.5;
            if (cleanSheets >= 5) ovrChange += 0.5;
            if (cleanSheets >= 10) ovrChange += 1;
            if (cleanSheets > 0) {
                attrs.ballControl = Math.min(99, (attrs.ballControl || 40) + 0.3 * Math.min(5, cleanSheets));
                attrs.composure = Math.min(99, (attrs.composure || 40) + 0.3 * Math.min(5, cleanSheets));
            }
        }
        ...
        addCommentary(`📈 <strong>END OF SEASON REVIEW</strong> — Avg rating ${avgRating.toFixed(2)}, OVR ${Math.round(currentOVR)} → ${Math.round(player.ovr)} (${ovrChange >= 0 ? '+' : ''}${ovrChange.toFixed(1)}).${abilityText} Value: ${formatValue(player.marketValue)}.`, 'decision-log');
    ...
    function midSeasonProgression() {
        ...
        const attrs = player.attributes || {};
        if (ovrChange > 0) {
            const points = Math.round(ovrChange * 2);
            const keys = ['finishing', 'passing', 'dribbling', 'ballControl', 'stamina', 'strength'];
            for (let i = 0; i < points; i++) {
                const key = keys[Math.floor(Math.random() * keys.length)];
                attrs[key] = Math.min(99, (attrs[key] || 40) + 0.5);
            }
        }
        ...
        player.marketValue = computeMarketValue();
        addCommentary(`🗓️ <strong>MID-SEASON REVIEW (MD ${MID_SEASON_WEEK})</strong> — Avg rating ${avgRating.toFixed(2)}, OVR ${Math.round(oldOVR)} → ${Math.round(player.ovr)} (${ovrChange >= 0 ? '+' : ''}${ovrChange.toFixed(1)}).${abilityText} Value: ${formatValue(player.marketValue)}.`, 'decision-log');
