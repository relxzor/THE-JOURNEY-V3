// ==================== training.js (FIXED - PREVENT OVR DROP) ====================
(function() {
    'use strict';
    ...
    if (drillsGrid) {
        ...
        drillsGrid.querySelectorAll('.drill-card').forEach(card => {
            card.addEventListener('click', () => {
                ...
                // BUT only update if the new OVR is higher than current
                const newOVR = computeOVRFromAttributes();
                const currentOVR = player.ovr || 65;
                
                // Only increase OVR, never decrease from training
                if (newOVR > currentOVR) {
                    player.ovr = newOVR;
                } else {
                    // If attributes improved but OVR didn't, keep current OVR
                    // This prevents OVR from dropping
                ...
                player.careerTimeline.push({
                    ...
                    event: 'Training',
                    detail: `Completed ${drill.name}. ${drill.label} improved from ${oldValue.toFixed(1)} to ${newValue.toFixed(1)}. OVR: ${player.ovr}`,
                    type: 'training'
                });
                ...
                // --- UPDATE UI ---
                showToast(`✅ ${drill.name} complete! +${drill.gain.toFixed(1)} ${drill.label} (OVR: ${player.ovr})`, 'success');
                updateProgress();
                updatePlayerInfo();
            });
        });
    ...
    // --- INIT ---
    // Ensure player.ovr is synced with attributes on load, but never decrease
    if (player.attributes) {
        const computedOVR = computeOVRFromAttributes();
        const currentOVR = player.ovr || 65;
        // Only update if computed is higher
        if (computedOVR > currentOVR) {
            player.ovr = computedOVR;
        } else {
            player.ovr = currentOVR;
        }
        player.marketValue = computeMarketValue();
