// ==================== training.js (HIGH IMPACT - 0.5 TO 1.0 GAIN) ====================
(function() {
    const player = JSON.parse(localStorage.getItem('theJourney_playerData') || 'null');
    if (!player) { window.location.href = 'index.html'; return; }

    const drills = [
        { name: 'Ball Control Drill', icon: 'fa-futbol', attr: 'ballControl', gain: 0.8, staminaCost: 8, fatigueGain: 6 },
        { name: 'Sprint Training', icon: 'fa-person-running', attr: 'sprintSpeed', gain: 0.7, staminaCost: 12, fatigueGain: 10 },
        { name: 'Passing Practice', icon: 'fa-share', attr: 'passing', gain: 0.8, staminaCost: 6, fatigueGain: 5 },
        { name: 'Shooting Drill', icon: 'fa-bullseye', attr: 'finishing', gain: 0.9, staminaCost: 9, fatigueGain: 7 },
        { name: 'Strength Workout', icon: 'fa-dumbbell', attr: 'strength', gain: 0.6, staminaCost: 14, fatigueGain: 12 },
        { name: 'Agility Ladder', icon: 'fa-shoe-prints', attr: 'agility', gain: 0.7, staminaCost: 8, fatigueGain: 6 },
        { name: 'Double Touch', icon: 'fa-rotate', attr: 'doubleTouch', gain: 1.0, staminaCost: 10, fatigueGain: 8 }
    ];
    ...
    drillsGrid.innerHTML = drills.map(d => `
        <div class="drill-card" data-drill="${d.attr}">
            ...
            <div class="drill-effect">+${d.gain.toFixed(1)} ${d.attr.replace(/([A-Z])/g,' $1').trim()}</div>
    ...
    drillsGrid.querySelectorAll('.drill-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            showToast(`✅ ${drill.name} complete! +${drill.gain.toFixed(1)} ${attr.replace(/([A-Z])/g,' $1').trim()}`, 'success');
            updateProgress();
        });
    });
