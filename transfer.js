// ==================== transfer.js (v8 - FIXED + REQUEST TRANSFER) ====================
(function() {
    'use strict';
    ...
    // --- LOAD OR GENERATE INTERESTS (persisted so offers don't vanish on reload) ---
    ...
    // --- NEGOTIATION (offers FROM other clubs) ---
    ...
    // --- EXECUTE TRANSFER (from accepted incoming offer) ---
    function executeTransfer(offer) {
        ...
        // Clear any pending player-initiated request and the interest pool (fresh market at new club)
    ...
    // ==================== REQUEST TRANSFER (player-initiated) ====================
    // Player asks their CURRENT club for permission to leave at a chosen window.
    // Current club's willingness depends on reputation, form, and how many times
    // the player has already been refused this season (refusals cost reputation
    // to discourage spamming requests).
    ...
    function evaluateClubResponse() {
        // Base willingness on reputation + form; refusals in the current request
        // cycle make the club progressively more annoyed (harder to approve).
    ...
    function checkRequestArrival() {
        ...
        if (currentWeek >= req.targetWeek) {
            // Window has arrived — surface interested clubs prominently.
            // The actual move still happens via accepting an offer below,
            // but we flag it so the player knows they're free to leave now.
    ...
    function renderRequestTransfer() {
        ...
        if (badge) badge.textContent = weeksLeft === 0 ? 'Window open!' : `${weeksLeft} week${weeksLef